app.controller('MainController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.getData = function() {
		$scope.loading = true;
		$http.get("http://localhost:8080/aeronave")
    ...
    $scope.delete = function(id) {
		$scope.loading = true;
		$http.delete("http://localhost:8080/aeronave/"+id)
		.then(function(response){ 
    ...
    $scope.save = function(id) {
		...
		$http.post("http://localhost:8080/aeronave/", $scope.myForm
        )
		.then(function(response){ 
			$scope.status = response.status;
            console.log($scope.status);
    ...
    $scope.edit = function(id) {
		$scope.loading = true;
		$http.patch("http://localhost:8080/aeronave/"+id, $scope.editForm
        )
		.then(function(response){ 
			$scope.status = response.status;
            console.log($scope.status);
